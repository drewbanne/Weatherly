...
import { Sun, Moon, Search, Wind, Droplet, Sunrise, Sunset, Eye, SunDim } from "lucide-react";
...
const mockWeatherData = {
  name: "Accra",
  main: { temp: 28, humidity: 77, feels_like: 32 }, // Added feels_like
...
const fetchWeather = async (city) => {
  ...
  if (city.toLowerCase() === "london" || city.toLowerCase() === "accra") {
    return { ...mockWeatherData, name: city, main: { ...mockWeatherData.main, temp: city.toLowerCase() === "london" ? 15 : 28, feels_like: city.toLowerCase() === "london" ? 12 : 32 } };
  }
...
const HighlightsCard = ({ weather }) => {
  ...
  return (
    <div className="p-6 rounded-2xl bg-white/60 dark:bg-gray-800/60 shadow-lg backdrop-blur-md">
      <h3 className="text-lg font-semibold mb-4 text-gray-800 dark:text-white">Today's Highlight</h3>
...
const ForecastCard = ({ forecast }) => {
  ...
  // Group forecasts by day and take the first entry for each day (around noon)
  const dailyForecasts = forecast.list.filter((item, index, self) => {
    const date = new Date(item.dt * 1000);
    const day = date.getDate();
    const existingDay = self.findIndex((el) => new Date(el.dt * 1000).getDate() === day);
    return index === existingDay;
...
// Search History Component (Static for now, can be made dynamic with localStorage)
const SearchHistory = () => {
  // Static mock data for search history
  const historyItems = [
    { city: "Lagos", temp: "32°/18°", condition: "Sunny", flag: "🇳🇬" },
    { city: "Cape Town", temp: "32°/18°", condition: "Sunny", flag: "🇿🇦" },
    { city: "Miami", temp: "32°/18°", condition: "Sunny", flag: "🇺🇸" },
  ];
    ...
    <div className="p-6 rounded-2xl bg-white/60 dark:bg-gray-800/60 shadow-lg backdrop-blur-md">
      ...
      <div className="space-y-3">
        {historyItems.map((item, index) => (
          <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-gray-100 dark:bg-gray-700 transition-transform hover:scale-[1.02]">
            <div className="flex items-center">
              <span className="text-2xl mr-2">{item.flag}</span>
              <div>
                <p className="font-medium text-gray-800 dark:text-white">{item.city}</p>
                <p className="text-sm text-gray-600 dark:text-gray-300">{item.condition}</p>
              </div>
            </div>
            <p className="text-lg font-bold text-gray-800 dark:text-white">{item.temp}</p>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-200 dark:from-gray-950 dark:via-gray-900 dark:to-gray-800 text-gray-900 dark:text-white font-inter antialiased transition-colors duration-300 ease-in-out">
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <section className="lg:col-span-2 space-y-6">
          <SearchHistory /> {/* Static search history for now */}
